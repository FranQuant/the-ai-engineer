...
Usage:
    python merge_fomc_corpus.py \\
        ...
        --minutes-corpus /tmp/fomc_minutes_candidate/\
fomc_minutes_1993_prewarsh.txt \\
        --minutes-manifest /tmp/fomc_minutes_candidate/\
fomc_minutes_1993_prewarsh_manifest.json \\
        --output-dir .
...
def merge(
    ...
    if stmt_actual_sha != stmt_manifest["corpus_sha256"]:
        raise SystemExit(
            f"Statements corpus file does not match its manifest's "
            f"recorded sha256 "
            f"(file={stmt_actual_sha}, "
            f"manifest={stmt_manifest['corpus_sha256']}) -- "
    ...
    if min_actual_sha != min_manifest["corpus_sha256"]:
        raise SystemExit(
            f"Minutes corpus file does not match its manifest's "
            f"recorded sha256 "
            f"(file={min_actual_sha}, "
            f"manifest={min_manifest['corpus_sha256']}) -- "
            f"re-run build_fomc_minutes_corpus.py or point at the matching "
            f"manifest."
        )
    ...
    if overlap:
        raise SystemExit(
            f"Duplicate document_id(s) across sources: {sorted(overlap)}"
        )
    ...
    combined_documents = [
        {**d, "source_corpus": "statements"}
        for d in stmt_manifest["documents"]
    ] + [
    ...
    validation_count = sum(
        1 for d in combined_documents if d["split"] == "validation"
    )
...
def run_self_tests(tmp_dir: Path) -> None:
    """Offline test using synthetic corpus/manifest pairs (no real files
    needed)."""
    stmt_text = (
        "<|fomc_statement|>\ndate: 2020-01-01\n\nHello statement.\n"
        "<|end_fomc_statement|>\n"
    ...
    corpus_path, manifest_path, manifest = merge(
        tmp_dir / "s.txt", tmp_dir / "s.json",
        tmp_dir / "m.txt", tmp_dir / "m.json", tmp_dir / "out"
    )
    ...
    assert (
        manifest["sources"]["minutes"]["known_coverage_gap"]
        == "test gap note"
    )

    # Integrity check: a hand-edited corpus (mismatched sha256) must be
    # rejected.
    (tmp_dir / "s.txt").write_text(stmt_text + "TAMPERED")
    try:
        merge(tmp_dir / "s.txt", tmp_dir / "s.json",
              tmp_dir / "m.txt", tmp_dir / "m.json", tmp_dir / "out2")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.self_test:
        ...
        return 0
    required = [args.statements_corpus, args.statements_manifest,
                args.minutes_corpus, args.minutes_manifest, args.output_dir]
    if any(r is None for r in required):
        print("error: all of --statements-corpus/--statements-manifest/"
              "--minutes-corpus/"
              "--minutes-manifest/--output-dir are required unless "
    ...
    print(f"corpus: {corpus_path} "
          f"({manifest['corpus_characters']:,} chars, "
